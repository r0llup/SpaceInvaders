...
/****************************************************************************************/
int OuvrirGrille(int Ligne,int Colonne,int TailleCarre,char* titre,int modeG)
{
...
void DessinerGrillage(GRILLE *pg)
{
  ...
  couleur = GRIS;
  if (pg->modeGrille == GRILLE_OFF) couleur = NOIR;
  ...
  if (pg->alive)
  {
    ...
    for (i=0 ; i<=pg->Colonne ; i++) DessineLigne(&(pg->tc),(i*pg->TailleCarre),0,(i*pg->TailleCarre),(hauteur-1),couleur);
    for (i=0 ; i<=pg->Ligne ; i++)   DessineLigne(&(pg->tc),0,(i*pg->TailleCarre),(largeur-1),(i*pg->TailleCarre),couleur);
  }
      ...
      for (l=0 ; l<pg->Ligne ; l++)
        for (c=0 ; c<pg->Colonne ; c++)
        {
          ...
          if (pg->tab[l][c] == MY_CANON1)
          {
            ...
            ya = 1 + 5*pg->TailleCarre/8 + l * pg->TailleCarre;
            xb = xa + 3*pg->TailleCarre/4 - 2;
            yb = ya + pg->TailleCarre/4 - 2;
            DessineRectangle(&(pg->tc),xa,ya,xb,yb,BLANC,PLEIN);

            // Tete du canon
            xa = 1 + 3*pg->TailleCarre/8 + c * pg->TailleCarre;
          ...
          if (pg->tab[l][c] == MY_VAISSEAU_AMIRAL)
          {
            ...
            // Corps horizontal
            xa = 1 + pg->TailleCarre/8 + c * pg->TailleCarre;
            ya = 1 + 2*pg->TailleCarre/8 + l * pg->TailleCarre;
            xb = xa + 6*pg->TailleCarre/8 - 2;
            yb = ya + 3*pg->TailleCarre/8 - 2;
            DessineRectangle(&(pg->tc),xa,ya,xb,yb,BLEU,PLEIN);

            // Corps vertical
            xa = 1 + 2*pg->TailleCarre/8 + c * pg->TailleCarre;
            ya = 1 + 1*pg->TailleCarre/8 + l * pg->TailleCarre;
            ...
            // jambe droite
            xa = 1 + 5*pg->TailleCarre/8 + c * pg->TailleCarre;
            ya = 1 + 1*pg->TailleCarre/8 + l * pg->TailleCarre;
            xb = xa + 1*pg->TailleCarre/8 - 2;
            yb = ya + 6*pg->TailleCarre/8 - 2;
            DessineRectangle(&(pg->tc),xa,ya,xb,yb,BLEU,PLEIN);
            ...
            // Pied gauche
            xa = 1 + 1*pg->TailleCarre/8 + c * pg->TailleCarre;
            ya = 1 + 6*pg->TailleCarre/8 + l * pg->TailleCarre;
            ...
            // oeil gauche
            xa = 1 + 2*pg->TailleCarre/8 + c * pg->TailleCarre;
            ya = 1 + 3*pg->TailleCarre/8 + l * pg->TailleCarre;
            xb = xa + 1*pg->TailleCarre/8 - 2;
            yb = ya + 1*pg->TailleCarre/8 - 2;
            DessineRectangle(&(pg->tc),xa,ya,xb,yb,ROUGE,PLEIN);
